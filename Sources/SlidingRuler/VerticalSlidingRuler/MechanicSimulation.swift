...
extension VerticalSlidingRuler {
  func applyInertia(initialVelocity: CGFloat) {
    func shiftOffset(by distance: CGSize) {
      let newOffset = directionalOffset(referenceOffset + distance)
      let newValue = value(fromOffset: newOffset)
    ...
    let finalOffset = referenceOffset + .init(horizontal: totalDistance)
  ...
  func applyInertialRubber(remainingVelocity: CGFloat) {
  ...
  func applyRubber(to offset: CGSize) -> CGSize {
