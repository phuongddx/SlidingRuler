...
extension View {
    ...
    public func styleFor(rulerDirection: RulerDirection) -> some View {
        switch rulerDirection {
            case .vertical:
                return environment(\.slidingRulerStyle, SlidingRulerStyleEnvironmentKey.verticalStyle)
            case .horizontal:
                return environment(\.slidingRulerStyle, SlidingRulerStyleEnvironmentKey.horizontalStyle)
        }
    }
}
...
private struct VerticalPanGesture: UIViewRepresentable {
    ...
    class Coordinator: NSObject, UIGestureRecognizerDelegate, VerticalPanGestureReceiverViewDelegate {
        ...
        init(_ beginTouch: @escaping () -> Void = {}, prematureEnd: @escaping () -> Void = {}, action: @escaping Action) {
        ...
        func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
            ...
            return abs(velocity.y) > abs(velocity.x) // Thay đổi để ưu tiên chuyển động dọc
        }
    ...
    func makeCoordinator() -> Coordinator {
        .init(beginTouch, prematureEnd: prematureEnd, action: action)
    }
